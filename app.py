...
def root_dir():
    if request.method == "POST":
        ...
        query_transaction_year = f"AND EXTRACT(YEAR from transaction_date) = \'{transaction_year}\'" \
            if transaction_year != "" else 'TRUE'

        connection = get_db_connection(db_file_path)
        cur = connection.cursor()

        full_query = f'select * from {table_name} where {query_member_name} {query_transaction_date}' \
                     f'{query_transaction_year} {query_company} '
        print(full_query)
